...
SAtwo_df <- clean(SAtwo, simplify = F)
...
centroid <- function(i, polys) {
    ...
    id <- i
    data.frame(long_c=ctr[1], lat_c=ctr[2], id = as.numeric(i))
}

centroids <-  purrr::map_df(seq_along(polys), centroid, polys=polys)


centroids <- seq_along(polys) %>% purrr::map_df(centroid, polys=polys) %>% as.tibble()

#join the centroids to full data set

SAtwos.df <- full_join(SAtwo_df %>% as.tibble(), centroids)
```
