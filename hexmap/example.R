...
aus_centroids <- assign_hexagons(centroid_data, grid)
...
(hexmap <- ggplot(ausGridAllocations) +
  stat_summary_hex(aes(x = hex_long, y = hex_lat, z = population, label=SA2_NAME16),
                   alpha = 0.75, binwidth = 0.23)
  )
