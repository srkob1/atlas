...
sa2_tidy <- sa2_tidy %>% left_join(select(sa2_hex, name, rnorm), by=c("SA2_NAME16"="name"))
...
sa2_tidy <- sa2_tidy %>% left_join(select(sa2_hex, name, rchisq), by=c("SA2_NAME16"="name"))
...
p4 <- ggplot(sa2_tidy) +
  ...
  theme_map()
p5 <- ggplot(sa2_tidy) +
  geom_polygon(aes(x=long, y=lat, group=group, fill = rchisq),
               colour=NA) +
  scale_fill_gradient("rchisq", low = "#184848", high = "#c0c030") +
  theme_map() + theme(legend.position="none")
