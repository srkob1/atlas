...
nac_colours <- c("#ff3500", "#ff9a64", "#fff4bc", "#aec6c7", "#33809d")
...
p2 <- ggplot(sa2_tidy %>% left_join(., sa2_colours, by = c("SA2_NAME16" = "name"))) +  geom_polygon(aes(x=long, y=lat, group=group,
               fill=colrs)) +
  scale_fill_manual(values = nac_colours)+
  theme_map() + 
  guides(fill=FALSE)
